...
public class ShapeEditor: Editor	
{
    ...
    protected void DrawPointsUI()
    {
		...
		PolygonThing poly = (PolygonThing)target;	
    ...
    protected void Drawpoints(PolygonThing poly, Event e)
    {	
	...
	protected void CheckDists(PolygonThing poly)
    {
		Fixture[] polys = poly.gameObject.GetComponents<FixturePoly>();
		Fixture[] chains = poly.gameObject.GetComponents<FixtureChainShape>();
		
		Fixture[] fixes = new Fixture[0];
		...
		{
			fixes = new Fixture[polys.Length + chains.Length];
	...
	protected void Movepoints(PolygonThing poly, Event e)
	{
		Vector3 mousepos = GetMousePos(e);
		int cID = GetCID();		
	...
	protected void Removepoints(PolygonThing poly, Event e)
	{
		Vector3 mousepos = GetMousePos(e);
		int cID = GetCID();		
	...
	protected void Insertpoints(PolygonThing poly, Event e)
    {
		Vector3 mousepos = GetMousePos(e);
		int cID = GetCID();	
							...
							,Color col, bool editme)
    {
		...
		Vector3[] points = ShapeTools.TransformPoints(transform,tran,pointsList);
		closest = 0;
		dist = 10f;
							...
							,out int closest,out float dist,Color col,bool editme)
	{
		...
		for (int i = 0; i < points.Length; i++)
		{
			...
			Handles.DotHandleCap(controlID,points[i]+tran,Quaternion.identity,HandleUtility.GetHandleSize(points[i]+tran)*0.07f, EventType.Repaint);
		}
		return isclosest;
	}
