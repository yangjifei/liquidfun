...
public class JointPulley : Joint
{
	...
	{		
		...
		B2ObjectPtr = API_Joint.CreatePulleyJoint(world,BodyA.GetComponent<Body>().GetPtr(),BodyB.GetComponent<Body>().GetPtr()	 
	...
	{			
		...
		Gizmos.DrawLine(transform.position+(Vector3)BodyAGroundOffset,BodyA.transform.position+
		 ShapeTools.RotatePoint((Vector3)BodyAAnchorOffset,BodyA.transform.rotation.eulerAngles.z,Vector3.zero));
		...
		+ ShapeTools.RotatePoint((Vector3)BodyBAnchorOffset,BodyB.transform.rotation.eulerAngles.z,Vector3.zero));
