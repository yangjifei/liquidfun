...
public abstract class PolygonThing : B2Object
{
	[HideInInspector]
	public bool Drawing;
	[HideInInspector]
	...
	{
		if (this.GetType() == typeof(FixturePoly) || this.GetType() == typeof(ParticleGroupPoly)
		    || this.GetType() == typeof(FixtureChainShape))
		{
	...
	protected IntPtr GetPolyShape(int NumberOfSides,float radius,Vector3 tran)
	{
		...
		PointsCopy = ShapeTools.TransformPoints(transform,tran,pointsList);
		
		//check complex			
		if (ShapeTools.CheckComplex(PointsCopy))
		{
			LogComplex();
			PointsCopy = ShapeTools.TransformPoints(transform,tran,ShapeTools.makePolyPoints(4,1f));
		}	
		
		//check anticlockwise			
		if (!ShapeTools.CheckAntiClockwise(PointsCopy))
		{
			Array.Reverse(PointsCopy); 
		}
		
		//check convex. If convex use decomposer and create multiple fixtures
		if(PointsCopy.Length > 8 || !ShapeTools.CheckConvex(PointsCopy))
		{					
			...
			if (bits.Count > 1)
			{
				...
				{
					InitialiseWithShape(ShapeTools.Initialise(ShapeTools.Vec2listToVec3Array(bits[i])));
				}
			}
			
			return ShapeTools.Initialise(ShapeTools.Vec2listToVec3Array(bits[0]));
		}
		
		return ShapeTools.Initialise(PointsCopy);
	}	
	public virtual bool CheckPointInPoly(Vector3 point)
	{
        point = transform.InverseTransformPoint(point);
		return ShapeTools.PointInPolygon(pointsList, point);
    }
	public bool CheckIntersecting(PolygonThing polygonThing)
	{
		ShapeTools.ArePolygonsIntersectingSAT(polygonThing.pointsList, pointsList);
		return false;
	}
}
